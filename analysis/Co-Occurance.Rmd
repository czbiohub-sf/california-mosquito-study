...
For bloodmeal categorization, we will use the curated list of taxonomic levels:

Aves
Bovidae (cows, sheep, goats, etc)
Cervidae (deer)
Leporidae (hare-like)
Procyon (Raccoon)

Association testing scale? Fix by geography.

Caveat: there is no external validation (eg, collecting mosquito off of animals).

Check if there are contigs going to Ruminantia and not something lower.


```{r}
for (query_taxon in c('Ruminantia',  'Aves', 'Procyon', 'Leporidae', 'Bovidae', 'Cervidae'))
{
  print(query_taxon)
