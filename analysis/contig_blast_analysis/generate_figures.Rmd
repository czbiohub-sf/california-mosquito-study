...
pkgs <- c("parallel", "dplyr", "tidyr", "readr", "magrittr", "ggplot2", "grid", "gridExtra", "stringr", "reutils", "rjson", "knitr", "taxize", "aws.s3")
...
blast_results_raw <- paste0("s3://czbiohub-mosquito/contig_quality_concat/blast_lca_", c("nr", "nt"), "_filtered.m9") %>%
  lapply(function (x) aws.s3::s3read_using(read_tsv, object = x)) %>%
  do.call(what=rbind, .) %>% 
