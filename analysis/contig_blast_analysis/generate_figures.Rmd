...
pkgs <- c("dplyr", "tidyr", "readr", "magrittr", "ggplot2", "grid", "gridExtra", "stringr", "knitr", "scales", "gtable", "rgdal", "broom")
...
base_plot <- 
  ...
  scale_y_continuous(labels=scales::percent, breaks=c(0.5), minor_breaks=c(0.25, 0.75), limits=c(0, 1), expand=c(0, 0)) +
  ...
  theme(legend.position="none",
        ...
        panel.grid.major.y=element_line(linetype=2, size=0.5, color="gray80"),
...
main_plot_grob <- arrangeGrob(legends_grob, species_label_grob, plot_grob, location_label_grob, ncol=1, heights=c(7, 3, 30, 2))
full_plot <- arrangeGrob(main_plot_grob, collection_site_grob, nrow=1, widths=c(7, 1))
ggsave(file.path(fig_dir, "fig3.pdf"), full_plot, width=unit(20, "in"), height=unit(8, "in"))
```
...
base_plot$data %>%
  ...
  mutate(collected_by=collected_by_dict[collected_by]) %>%
...
ggsave(file.path(fig_dir, "../figS4/virus_prev_abundance.pdf"), select_virus_prev_abund_plot,
...
select_virus_prev_abund_plot$data %>%
  ...
  pivot_wider(id_cols=c(mosquito_species, collection_lat, collection_long, sci_name, bulk_abundance, num_mosquitos), values_from=read_prop, names_from=mosquito_id, names_prefix="mosquito_") %>%
