...
curated_contigs <- filter(all_curated_contigs, group %in% c("Virus", "Wolbachia", "Other Eukaryotes") | family=="Onchocercidae") %>%
...
curated_contigs_low_abundance <- curated_contigs %>%
  group_by(sample, group) %>% 
  summarize(sum=sum(read_prop)) %>% 
  filter(sum<0.01)

low_abundance_df <- 
  filter(curated_contigs, sample %in% curated_contigs_low_abundance$sample, group!="Virus") %>% 
  select(sample, family) %>% 
  distinct() %>%
  mutate(present=TRUE) %>%
  rename(low_abundance_family=family) %>%
  left_join(curated_contigs, ., by="sample") %>%
