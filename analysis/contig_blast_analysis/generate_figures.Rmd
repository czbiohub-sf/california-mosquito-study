...
```{r save-data-supp}
prev_abundance_plot$data %>%
  mutate(mosquito_species=paste(ska_genus, ska_species)) %>%
  mutate(collection_long=round(collection_long, 2), round(collection_lat, 2)) %>%
  mutate(sample=paste0("CMS00", sample_short)) %>%
  select(-ska_genus, -ska_species, -sample_short) %>%
  rename(in_panel_B=highlight) %>%
  write_tsv(file.path(fig_dir, "../supplementary/virus_prev_abundance_data.tsv"))
```
