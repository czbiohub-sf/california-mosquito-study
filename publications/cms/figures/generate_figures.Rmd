...
close_viral_prev_abund_vignettes <- list(high_abundance_low_prev = filter(close_viral_prev_abundance, abundance>0.1, prevalence<0.25)$taxid[1],
     low_abundance_high_prev = filter(close_viral_prev_abundance, abundance<0.1, prevalence>0.9)$taxid[1],
     high_abundance_high_prev = filter(close_viral_prev_abundance, abundance>0.3, prevalence>0.5, prevalence<0.75)$taxid[1],
     low_abundance_low_prev = filter(close_viral_prev_abundance, abundance<1e-5) %>% with(taxid[order(prevalence)][2]))
