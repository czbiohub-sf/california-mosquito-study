...
westvalley_names <- c("Chino", "Chino Hills", "Montclair", "Ontario", "Rancho Cucamonga", "Upland")
# https://data.ca.gov/dataset/ca-geographic-boundaries
ca_shp <- file.path(data_dir, "shapefiles", paste0("CA_", c("State", "Counties", "Places"))) %>%
  lapply(., readOGR, stringsAsFactors=FALSE) %>%
  lapply(., spTransform, CRS("+proj=longlat +datum=WGS84")) %>%
  setNames(., sapply(., function (x) grep("NS$", names(x), value=TRUE) %>% gsub("NS", "", .) %>% capitalize_first()))
coachellavalley_names <- ca_places_shp@data %>% select(INTPTLON, INTPTLAT) %>%
  apply(1, as.numeric) %>% 
...
ca_shp$Areas <- ca_shp$County %>%
  `[`(., .$NAME %in% c("Alameda", "Placer", "San Diego"), )
ca_shp$Areas@plotOrder <- 1:5
ca_shp$Areas@data %<>% rbind(., .[1:2, ])
places_data <- slice(ca_shp$Place@data, match(c("Ontario", "Coachella"), NAME)) %>%
...
ca_shp$Areas@polygons %<>% 
  c(lapply(list(westvalley_names, coachellavalley_names), function (x) {
    unionSpatialPolygons(ca_shp$Place[ca_shp$Place$NAME %in% x, ], rep(1, length(x))) %>%
      spTransform(., CRS("+proj=longlat +datum=WGS84")) %>%
      `@`(., "polygons") %>% 
      `[[`(1)
    }))
for (i in 1:length(slot(ca_shp$Areas, "polygons"))){
...
polygon_plots <- lapply(with(ca_shp$Areas@data, id[order(order)]), function (x) {
  ...
  create_map(filter(cms_all_data, location_name==place_name), polygon=plot_data, polygon_col=filter(ca_shp$Areas@data, id==x)$colours) +
    ggtitle(paste0(filter(ca_shp$Areas@data, id==x)$NAME, ":\nNumber of sites: ", length(sites), "\nn: ", n)) +
    theme(plot.title=element_text(hjust=0.5))
})
# polygon_meta <- sapply(polygons, function (x) {
#     x$data[, c("long", "lat")] %>% lapply(range) %>% sapply(diff)
# }) %>%
#   t() %>%
#   data.frame() %>%
#   mutate(group=c(1, 1, 2, 2, 2)) %>%
#   data.frame(slice(ca_shp$Areas@data, order(order)), .) %>%
#   rename(long_range=long, lat_range=lat)
...
```{r create-data-frames}
 + 
  geom_polygon(polygons[[1]]$data, aes(x=long, y=lat, group=group), fill=filter(ca_shp$Areas@data, id==x)$colours)
